...
def _replace_pattern_with_ns(s, kw, pattern=re.compile(r'\$(\w+)')):
    """
...
def _replace_pattern_with_bash(source, pattern=re.compile(r'\$\{(\w+)\}')):
    """
    Replace delimited keys in a string with the evaluation of bash commands (command is in the square bracket).
...
class SqlMagic(Magics, Configurable):
    ...
    displaylimit = Int(0, config=True,
    ...
    raise_exceptions = Bool(False, config=True, help="Define if exceptions should be thrown or just be printed "
                                                     "(default to False: just be printed)")
